...
#
# Usage (as hook):
#     echo '{"tool_input":{"file_path":".claude/skills/commit/skill.yaml"}}' | ./check-skill-structure.sh
#
# INPUTS:
...
if ! command -v task >/dev/null 2>&1; then
    # Task runner not installed, skip validation
    exit $SUCCESS
fi

# Run structural validation (YAML parsing + required files)
if ! task claude:audit-skills >/dev/null 2>&1; then
    echo "Skill structure validation failed" >&2
    echo "" >&2
    echo "Run 'task claude:audit-skills' for details:" >&2
    task claude:audit-skills 2>&1 | head -20 >&2
    exit $ERR_VALIDATION
fi
